...
abstract class WebSocketServer {
	...
	protected $userClass = 'WebSocketUser', // redefine this if you want a custom user class. The custom user class should inherit from WebSocketUser.
				...
				$heldMessages							= [],
				$interactive							= true,
				$headerOriginRequired					= false,
				$headerSecWebSocketProtocolRequired	 	= false,
				$headerSecWebSocketExtensionsRequired 	= false;
	...
	public function selfLoop (bool $loop) {
		$this->selfLoop = $loop;
	}
		...
		do {
			...
			foreach ($read as $socket) {
				if ($socket == $this->master) {
					...
					if (($numBytes = @socket_recv ($socket, $buffer, $this->maxBufferSize, 0)) === false) {
						$sockErrNo = socket_last_error ($socket);
						switch ($sockErrNo) {
							case 102:		// ENETRESET		-- Network dropped connection because of reset
							case 103:		// ECONNABORTED		-- Software caused connection abort
							case 104:		// ECONNRESET		-- Connection reset by peer
							case 10054:		// WSAECONNRESET Gotta love window (tm)
							...
							case 125:		// ECANCELED		-- Operation canceled
								...
								break;
							default:
									$this->stderr ("Unknown socket error code ($sockErrNo). Socket error: ".socket_strerror ($sockErrNo));
								break;
						}
					} else if ($numBytes === 0) {
	...
	protected function doHandshake($user, $buffer) {
		# TODO: Better handling for first row.
